...
def get_daily_km_by_unit(filters: dict[str, Any] | None = None) -> pd.DataFrame:
    ...
    gps = gps.copy()
    gps["km"] = pd.to_numeric(gps.get("km"), errors="coerce").fillna(0)
    gps["duracion_reportada_seg"] = pd.to_numeric(gps.get("duracion_reportada_seg"), errors="coerce").fillna(0)
    gps["inicio_dt"] = pd.to_datetime(gps.get("inicio_datetime"), errors="coerce")
    gps["fin_dt"] = pd.to_datetime(gps.get("fin_datetime"), errors="coerce")
    gps["fecha"] = pd.to_datetime(gps["fecha"], errors="coerce").dt.date.astype(str)

    out = gps.groupby(["fecha", "unidad_id", "placas_catalogo"], as_index=False, dropna=False).agg(
        km_gps=("km", "sum"),
        movimientos=("id", "count"),
        horas_movimiento=("duracion_reportada_seg", lambda s: s.sum() / 3600.0),
        primer_dt=("inicio_dt", "min"),
        ultimo_dt=("fin_dt", "max"),
