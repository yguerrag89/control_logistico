    ...
    with get_connection() as conn:
        cur = conn.execute(
            ...
            INSERT INTO rutas (
                ...
                hora_regreso_reportada, estado_ruta, observaciones_generales
            ) VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (
                data["fecha"], data["unidad_id"], data["conductor_id"],
                data.get("hora_salida_reportada"), data.get("hora_regreso_reportada"),
                data.get("estado_ruta", "Abierta"), data.get("observaciones_generales"),
            ),
        )
...
def update_route(route_id: int, data: dict[str, Any], motivo: str | None = None, comentario: str | None = None, usuario: str | None = None) -> None:
    fields = ["fecha", "unidad_id", "conductor_id", "hora_salida_reportada", "hora_regreso_reportada", "estado_ruta", "observaciones_generales"]
    with get_connection() as conn:
        ...
        conn.execute(
            ...
            (
                data["fecha"], data["unidad_id"], data["conductor_id"],
                data.get("hora_salida_reportada"), data.get("hora_regreso_reportada"),
                data.get("estado_ruta", "Abierta"), data.get("observaciones_generales"), route_id,
            ),
        )
        if critical_changed:
...
def list_routes(filters: dict[str, Any] | None = None) -> pd.DataFrame:
    filters = filters or {}
    sql = """
        SELECT r.*, u.placas, u.tipo_unidad, d.nombre AS conductor_nombre,
               COUNT(e.id) AS entregas_capturadas
        FROM rutas r
        JOIN unidades u ON u.id = r.unidad_id
        JOIN conductores d ON d.id = r.conductor_id
        LEFT JOIN ruta_entregas e ON e.ruta_id = r.id AND e.activo = 1
        WHERE r.activo = 1
    """
    params: list[Any] = []
...
def route_summary_metrics() -> dict[str, Any]:
    with get_connection() as conn:
        return {
            "rutas_abiertas": conn.execute("SELECT COUNT(*) AS n FROM rutas WHERE activo=1 AND estado_ruta='Abierta'").fetchone()["n"],
            "rutas_pendientes_gps": conn.execute("SELECT COUNT(*) AS n FROM rutas WHERE activo=1 AND estado_ruta='Cerrada pendiente de GPS'").fetchone()["n"],
