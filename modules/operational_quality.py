...
def route_time_inconsistencies(filters: dict[str, Any] | None = None) -> pd.DataFrame:
    ...
    sql = """
        ...
        WHERE COALESCE(e.activo,1)=1 AND COALESCE(r.activo,1)=1
          AND (
...
def route_state_inconsistencies() -> pd.DataFrame:
    sql = """
        SELECT r.id AS ruta_id, r.fecha, u.placas, r.estado_ruta,
               COUNT(e.id) AS entregas,
               SUM(CASE WHEN e.estado_conciliacion_gps IN ('Asociada exacta','Asociada cercana','Asociada manualmente') THEN 1 ELSE 0 END) AS entregas_asociadas,
               SUM(CASE WHEN e.estado_conciliacion_gps IN ('Sin GPS asociado','Pendiente de GPS','Conflicto') THEN 1 ELSE 0 END) AS entregas_no_conciliadas
        FROM rutas r
        JOIN unidades u ON u.id = r.unidad_id
        LEFT JOIN ruta_entregas e ON e.ruta_id = r.id AND COALESCE(e.activo,1)=1
        WHERE COALESCE(r.activo,1)=1
        GROUP BY r.id
        HAVING (r.estado_ruta LIKE 'Conciliada%' AND COALESCE(entregas_no_conciliadas,0) > 0)
...
def repair_all_route_statuses(usuario: str | None = "admin") -> int:
    ...
    with get_connection() as conn:
        route_ids = [int(r[0]) for r in conn.execute("SELECT id FROM rutas WHERE COALESCE(activo,1)=1").fetchall()]
