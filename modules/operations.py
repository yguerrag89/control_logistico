...
def route_closure_snapshot(route_id: int) -> dict[str, Any]:
    with get_connection() as conn:
        ...
        deliveries = conn.execute(
            """
            SELECT e.*, COUNT(ev.id) AS evidencias
            FROM ruta_entregas e
            LEFT JOIN ruta_entrega_evidencias ev ON ev.entrega_id=e.id AND COALESCE(ev.estado_evidencia,'activo')='activo'
...
def deliveries_without_destination() -> pd.DataFrame:
    with get_connection() as conn:
        rows = conn.execute(
            ...
            JOIN rutas r ON r.id=e.ruta_id
            JOIN unidades u ON u.id=r.unidad_id
            WHERE COALESCE(e.activo,1)=1 AND e.destino_id IS NULL
