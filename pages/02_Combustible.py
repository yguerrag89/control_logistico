...
def _render_driver_fuel_capture(ctx: dict) -> None:
    ...
    with st.form("driver_fuel_form", clear_on_submit=False, border=True):
        ...
        fecha_carga = st.date_input("Fecha de carga", value=datetime.now().date())
        hora_carga_time = st.time_input("Hora de carga", value=datetime.now().time().replace(second=0, microsecond=0))
...
"""
    ...
    payload = {
        ...
        "hora_carga": hora_carga_time.strftime("%H:%M"),
